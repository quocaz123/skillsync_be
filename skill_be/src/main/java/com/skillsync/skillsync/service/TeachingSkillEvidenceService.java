...
public class TeachingSkillEvidenceService {
    ...
    public TeachingSkillEvidenceResponse create(UUID teachingSkillId, TeachingSkillEvidenceRequest request) {
        UserTeachingSkill teachingSkill = teachingSkillRepository.findById(teachingSkillId)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "Teaching skill not found"));

        User currentUser = userService.getCurrentUser();

        if (!teachingSkill.getUser().getId().equals(currentUser.getId()))
            throw new AppException(ErrorCode.FORBIDDEN, "Bạn không có quyền thêm minh chứng cho kỹ năng này");
    ...
    public void delete(UUID evidenceId) {
        TeachingSkillEvidence evidence = evidenceRepository.findById(evidenceId)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "Evidence not found"));

        User currentUser = userService.getCurrentUser();

        if (!evidence.getTeachingSkill().getUser().getId().equals(currentUser.getId()))
            throw new AppException(ErrorCode.FORBIDDEN, "Bạn không có quyền xóa minh chứng này");
    ...
    private void validateEvidenceRequest(TeachingSkillEvidenceRequest request) {
        ...
        if (request.getEvidenceType() == null)
            throw new AppException(ErrorCode.INVALID_REQUEST, "evidenceType không được để trống");
        if (request.getTitle() == null || request.getTitle().isBlank())
            throw new AppException(ErrorCode.INVALID_REQUEST, "title không được để trống");

        boolean hasFile = request.getFileUrl() != null && !request.getFileUrl().isBlank();
        boolean hasUrl = request.getExternalUrl() != null && !request.getExternalUrl().isBlank();

        if (!hasFile && !hasUrl)
            throw new AppException(ErrorCode.INVALID_REQUEST, "Phải có file hoặc externalUrl");
    }
}
